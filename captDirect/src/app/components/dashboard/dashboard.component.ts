...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  sessionId: number = 47 // ID de la session à afficher
  segments: any[] = [];
  username: string = '';
...
/**
 * Appelle le back pour générer le SRT fusionné
 * et déclenche le téléchargement côté client.
 */
onExportSrt(): void {
  console.log('onExportSrt appelé, envoi de la requête export-srt');
  this.sessionService.exportSrt(this.sessionId)
    .subscribe({
      next: (blob: Blob) => {
