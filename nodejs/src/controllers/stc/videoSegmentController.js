...
async function handleUserDisconnection(req, res) {
  ...
  try {
    ...
    const updatedSegments = segments.map((segment) => {
      ...
      // Vérifier si le segment est assigné à l'utilisateur déconnecté
      if (parsedSegment.status === "assigned" && parsedSegment.user_id === user_id) {
        // Segments en cours ne doivent pas être redistribués
        if (parsedSegment.status === "in_progress") {
          console.log(
            `Segment ${parsedSegment.segment_id} est en cours. Non redistribué.`
          );
          return parsedSegment; // Ne modifie pas les segments en cours
        }
  
        // Sinon, le segment est marqué comme "available" pour redistribution
        parsedSegment.status = "available";
        delete parsedSegment.user_id; // Supprimer la référence à l'utilisateur
    ...
    if (segmentsToRedistribute.length > 0) {
      ...
      // Réassigner les segments disponibles aux utilisateurs connectés
      const totalUsers = connectedUsers.length;
      segmentsToRedistribute.forEach((segment, index) => {
        const userToAssign = connectedUsers[index % totalUsers].user_id;
        segment.user_id = userToAssign; // Assigner l'utilisateur
        segment.status = "assigned"; // Changer le statut en "assigned"
      });
