...
const VideoSegmentModel = require("./VideoSegmentModel");
...
class SegmentUserModel extends Model {
  ...
  async assignUserToSegment(user_id, segment_id) {
    // Vérifier que le segment est "available"
    const segment = await VideoSegmentModel.findById(segment_id);
    if (!segment || (segment.status !== "available" && segment.status !== "assigned")) {
      throw new Error(`Le segment ${segment_id} n'est pas disponible pour l'assignation.`);
    }
  ...
  async deleteAssignmentsBySegment(segment_id) {
    const query = `
      DELETE FROM ${this.tableName}
      WHERE segment_id = $1;
    `;
    await pool.query(query, [segment_id]);
    console.log(`Assignations supprimées pour le segment ${segment_id}.`);
  }
